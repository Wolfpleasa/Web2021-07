<template>
  <div>
    ...
    <Menu
      ...
      :isFocusCustomer="isFocusCustomer"
    />
      ...
      <div class="content-header">
        ...
        <div class="d-flex">
          <ButtonIcon
            ...
            iconName="icon-add"
            buttonText="Nhân bản khách hàng"
            id="btnDuplicate"
      ...
      <div class="filter">
        <div class="d-flex">
          ...
          <Dropdown
            ...
            defaultName="Tất cả nhóm khách hàng"
...
export default {
  ...
  data() {
    return {
      ...
      // Silde
      active: false,
      // Tô đậm danh mục khách hàng
      isFocusCustomer: true,
  ...
  methods: {
    ...
    btnDeleteOnClick() {
      ...
      if (count > 1) {
        me.warning = `Xóa thông tin của ${count} khách hàng`;
        me.warningText = `Xác nhận xóa thông tin của <b>${count} khách hàng </b> này không`;
      } else {
        let fullname = "";
        ...
        me.warningText = `Xác nhận xóa thông tin của khách hàng <b>${fullname}</b>`;
      }
    ...
    btnDuplicateOnClick() {
      ...
      if (count > 1) {
        me.warning = `Nhân bản thông tin của ${count} khách hàng`;
        me.warningText = `Xác nhận nhân bản thông tin của <b>${count} khách hàng </b> này không`;
      } else {
        let fullname = "";
        this.isSelected.forEach((selected, index) => {
          if (selected) {
        ...
        me.warningText = `Xác nhận nhân bản thông tin của khách hàng <b>${fullname} </b>`;
      }
          ...
          (async () => {
            for (var i = 0; i < me.isSelected.length; i++) {
              if (me.isSelected[i]) {
                ...
                // gán mã khách hàng vừa lấy dc vào mã khách hàng của khách hàng được chọn để tạo ra 1 khách hàng nhân bản
                employeeDuplicated["EmployeeCode"] = newCode;
                // gửi khách hàng nhân bản lên để thêm vào db
    ...
    /**
     * Hàm lấy khách hàng theo id khách hàng
     * Ngọc 6/8/2021
     */
    getInfo(employeeId) {
      return new Promise((resolve) => {
    ...
    /**
     * Hàm được gọi khi thay đổi page hoặc số lượng khách hàng/trang
     * Ngọc 12/8/2021
     */
    ...
    loadDataTable() {
      let me = this;
      me.employees = [];
      let url = `https://localhost:44373/api/v1/Employees/Paging?pageSize=${me.entityPerPage}&pageNumber=${me.currentPageNumber}`;
    ...
    btnDialogCancelOnClick() {
      ...
      me.warningText = `Bạn có chắc muốn đóng thông tin form ${custom} <b>"Thông tin khách hàng"</b> hay không`;
    ...
    /**
     * Hàm bấm lưu được CustomerDetail gửi lên
     * Ngọc 30/07/2021
     */
    btnSaveOnClick(employeeFullName) {
      ...
      me.WarningHasDnone = false;
      me.idPopup = "notify-popup";
      me.warning = "Lưu thông tin khách hàng ";
      me.warningText = `Xác nhận ${custom} khách hàng <b>${employeeFullName}</b> `;
      me.btnCancelText = "Hủy";
      me.btnConfirmText = "Lưu";
      me.bonusClass = "w-100";
      me.notifyMode = 0;
